...
<ul class="topnav">
    ...
    <li><a class="active" href="searchCategoryCommand">Wyszukiwanie produktu</a></li>
...
<c:if test="${searchTerm.length() != 0 && searchTerm.length() < 3}">Szukana fraza musi miec co najmniej 3 znaki! Spróbuj ponownie.</c:if>
<c:if test="${searchTerm.length() >= 3 && searchResults.size() == 0}">Przepraszamy, nie znaleziono kategorii!</c:if>
<jsp:useBean id="searchResults" scope="request" type="java.util.List<pl.infoshareacademy.webapp.searchCategoryCommandWeb.SearchResult>" />
